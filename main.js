...
const poemMap = {
    'entrance':         'The house has been acting differently lately.',
    'mirror':           'The mirror fogs before I touch it. A small, startled ripple follows me down the hallway.',
    'hallway':          "I tell myself it's nothing—just the house remembering its own humidity—but the floorboards swell when I laugh, and once the toaster coughed up a single wet feather.",
    'window':           "Outside, the sky hangs low, loose as a pocket.\nI tuck my doubts inside it.\nThe wind answers in part whistle,\npart apology.",
    'bus-stop':         "At the bus stop, a stray dog circles me twice.\nIts nose is always damp.\nEverything I touch grows a little softer,\nas if the world is slowly thawing under my hands.\nMy shadow trails behind me like a dog.",
    'ghost-silhouette': "Someone asks if I'm okay. I say yes.",
    'bathroom':         "I cup my hands under the faucet and drink until the water grows tired of me. It presses itself along the edges of the mouth, testing for escape. I don't know if it wants a map.",
    'ghost-return':     "Some nights there's a ghost that settles beneath my ribs.",
    'window-cracked':   "Other nights, it slips out the window and circles the yard, as if checking that the world is still intact. I still don't know if I'm the tide or just the rumor of one. But I leave the window cracked a little,",
    'hallway-return':   "so when it returns, it knows there's a way back in.",
};
...
const TEXT_POSITIONS = {
    ...
    'bus-stop':         { right: '2%', top: '10%' },
    'ghost-silhouette': { left: '38%', top: '25%', noWrap: true },
    'bathroom':         { right: '2%', top: '36%' },
    'ghost-return':     { right: '2%', top: '50%' },
    'window-cracked':   { right: '2%', top: '72%' },
...
const ROOM_HIGHLIGHT = {
    'window':         'window-cracked',
    'window-cracked': 'hallway-return',
};

const SCENE_BG = {
    'window':           0xB84820,
    'bus-stop':         0x4A2858,
    'ghost-silhouette': 0x1C1040,
    'bathroom':         0x120B28,
    'ghost-return':     0x0C0820,
    'window-cracked':   0x080614,
    'hallway-return':   0x060412,
};
...
const allMeshes         = [];
const allGlassPanes     = [];
const ghostFigureMeshes = [];
const nightStars        = [];
let bedroomGlass = null;
let busStopDog   = null;
let bgTarget     = new THREE.Color(0xCCE8F8);
...
// Shared material factories used by all room-detail blocks
...
// Registers detail meshes, fades them in after `delay` ms
function roomDetail(delay, builder) {
    const items = [];
    const reg = (m, op) => {
        m.userData.baseOp = op; m.userData.targetOp = 0;
        allMeshes.push(m); scene.add(m); items.push(m); return m;
    };
    builder(reg);
    setTimeout(() => items.forEach(m => { m.userData.targetOp = m.userData.baseOp; }), delay);
}

// 5-face box (top + front + sides + wireframe) — used by every furnished room
function makeBox(reg) {
    return (x, y, z, w, h, d, tC, fC, sC, eC, op) => {
        ...
        const e = reg(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(w, h, d)), ln(eC)), op+0.02);
        e.position.set(x, y+h/2, z); e.renderOrder = 3;
    };
}
...
// Stars (visible behind bedroom window during 'window' step)
{
    const pos = [];
    ...
    const starMesh = new THREE.Points(starGeo,
        new THREE.PointsMaterial({ color: 0xFFFFFF, size: 0.05, transparent: true, opacity: 0, sizeAttenuation: true, depthTest: false })
    );
...
// Bus stop
{
    ...
    g.add(addMesh(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
    ...
    [[-sw/2, -sd/2], [sw/2, -sd/2], [-sw/2, sd/2], [sw/2, sd/2]].forEach(([px, pz]) => {
        g.add(addMesh(new THREE.Line(
            new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(px, -sh/2, pz), new THREE.Vector3(px, sh/2, pz)]),
            lineM()
        ), 1.0));
    });
    ...
    g.add(addMesh(new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(-0.025, 0.04, -sd/2 + 0.02), new THREE.Vector3( 0.725, 0.04, -sd/2 + 0.02),
        new THREE.Vector3( 0.725, 0.56, -sd/2 + 0.02), new THREE.Vector3(-0.025, 0.56, -sd/2 + 0.02),
    ]), lineM()), 0.88));
    ...
    const dogMesh = new THREE.Mesh(new THREE.ShapeGeometry(dogShape),
...
// Ghost silhouette in attic window
{
    ...
    // Clipping planes confine the figure to the attic window bounds
    const gfClip = [
    ...
    const glow = new THREE.Mesh(new THREE.PlaneGeometry(0.82, 0.68),
        new THREE.MeshBasicMaterial({ color: 0xFFF4D8, transparent: true, opacity: 0, depthTest: false, clippingPlanes: gfClip }));
    glow.position.set(gfX, gfY, gfZ - 0.05);
    gfReg(glow, 0.65, 10);

    const halo = new THREE.Mesh(new THREE.SphereGeometry(0.40, 10, 8),
        new THREE.MeshBasicMaterial({ color: 0xFFEEC8, transparent: true, opacity: 0, depthTest: false, clippingPlanes: gfClip }));
    ...
    figShape.bezierCurveTo(-0.24, 0.02, -0.10, 0.06, -0.06, 0.08);
    ...
    figShape.bezierCurveTo( 0.10, 0.06,  0.24, 0.02,  0.26, -0.04);
    figShape.lineTo( 0.26, -0.12);
    figShape.closePath();

    const figMesh = new THREE.Mesh(new THREE.ShapeGeometry(figShape),
        new THREE.MeshBasicMaterial({ color: 0x0A0618, transparent: true, opacity: 0, depthTest: false, clippingPlanes: gfClip }));
    figMesh.position.set(gfX, gfY - 0.22, gfZ);
    gfReg(figMesh, 0.92, 13);

    [-0.075, 0.075].forEach(xOff => {
        const eye = new THREE.Mesh(new THREE.CircleGeometry(0.030, 8),
            new THREE.MeshBasicMaterial({ color: 0xC8DCF8, transparent: true, opacity: 0, depthTest: false, clippingPlanes: gfClip }));
...
// Roof
{
    ...
    [[0,1],[1,2],[2,0],[3,4],[4,5],[5,3],[0,3],[1,4],[2,5]].forEach(([a,b]) => {
        ...
        l.userData.isRoof = true; l.renderOrder = 10;
    ...
    // Attic gable window
    {
        ...
        const awW = 0.80, awH = 0.65, awDelay = 1500;
        ...
        awRegLine([new THREE.Vector3(L,B,z2), new THREE.Vector3(R,B,z2), new THREE.Vector3(R,T,z2), new THREE.Vector3(L,T,z2), new THREE.Vector3(L,B,z2)]);
        ...
        awRegLine([new THREE.Vector3(L-0.07,T,z2), new THREE.Vector3(awX,T+0.28,z2), new THREE.Vector3(R+0.07,T,z2)]);
        awRegLine([new THREE.Vector3(L-0.07,B-0.04,z2), new THREE.Vector3(R+0.07,B-0.04,z2)]);
    }
}

// Exterior corner edge lines
{
    const dh = D / 2;
    [[-4.5,-dh],[-4.5,dh],[4.5,-dh],[4.5,dh]].forEach(([ex,ez]) => {
        const l = new THREE.Line(
            new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(ex,0,ez), new THREE.Vector3(ex,9,ez)]),
...
// hallway ceiling pendant
detail([[[0,5.85,-0.8],[0,5.2,-0.8]]], 660);
addBulb(0, 5.05, -0.8, 660);

// bathroom: sink + faucet + toilet
roomDetail(740, reg => {
    const sx = 3, sy = 3.30, sz = -1.45, sw = 0.68, sh = 0.28, sd = 0.50;

    const front = new THREE.Mesh(new THREE.PlaneGeometry(sw, sh), face(0xF2EEE9));
    front.position.set(sx, sy, sz+sd/2); reg(front, 1.0).renderOrder = 3;

    const lside = new THREE.Mesh(new THREE.PlaneGeometry(sd, sh), face(0xC8C4BE));
    lside.rotation.y = -Math.PI/2; lside.position.set(sx-sw/2, sy, sz); reg(lside, 1.0).renderOrder = 3;

    const rside = new THREE.Mesh(new THREE.PlaneGeometry(sd, sh), face(0xC8C4BE));
    rside.rotation.y = Math.PI/2; rside.position.set(sx+sw/2, sy, sz); reg(rside, 1.0).renderOrder = 3;

    const bface = new THREE.Mesh(new THREE.PlaneGeometry(sw, sh), face(0xB8B4AE));
    bface.rotation.y = Math.PI; bface.position.set(sx, sy, sz-sd/2); reg(bface, 1.0).renderOrder = 3;

    const bfaceInner = new THREE.Mesh(new THREE.PlaneGeometry(sw, sh), face(0xF2EEE9));
    bfaceInner.position.set(sx, sy, sz-sd/2+0.01); reg(bfaceInner, 1.0).renderOrder = 3;

    const sinkFloor = new THREE.Mesh(new THREE.PlaneGeometry(sw, sd), face(0xD4CEC8));
    sinkFloor.rotation.x = -Math.PI/2; sinkFloor.position.set(sx, sy-sh/2, sz); reg(sinkFloor, 1.0).renderOrder = 3;
    ...
    edges.position.set(sx, sy, sz); reg(edges, 0.95).renderOrder = 3;

    const water = new THREE.Mesh(new THREE.PlaneGeometry(sw-0.10, sd-0.10), face(0x3ABDD8));
    water.rotation.x = -Math.PI/2; water.position.set(sx, sy-sh/2+0.06, sz); reg(water, 0.92).renderOrder = 3;

    const drain = new THREE.Mesh(new THREE.CircleGeometry(0.055, 8), face(0x9A8A78));
    drain.rotation.x = -Math.PI/2; drain.position.set(sx, sy-sh/2+0.065, sz); reg(drain, 0.90).renderOrder = 3;

    // faucet
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(3.0,3.44,-1.70), new THREE.Vector3(3.0,3.72,-1.70),
    ]), ln(0x5C5048)), DET_OP).renderOrder = 3;
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(2.92,3.68,-1.70), new THREE.Vector3(3.08,3.68,-1.70),
    ]), ln(0x5C5048)), DET_OP).renderOrder = 3;
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(3.0,3.72,-1.70), new THREE.Vector3(3.0,3.76,-1.58), new THREE.Vector3(3.0,3.62,-1.45),
    ]), ln(0x5C5048)), DET_OP).renderOrder = 3;

    // toilet
    const box = makeBox(reg);
    const bfy = 3.0, tx = 3.66, tW = 0.40, tH = 0.40, tD = 0.52, tz = -1.48;
    ...
    box(tx, bfy+tH, tz-tD/2-0.09, tW-0.02, 0.26, 0.18,    pC, pF, pS, pE, 1.0);
});
...
roomDetail(500, reg => {
    ...
    rug.rotation.x = -Math.PI/2; rug.position.set(lx, fy+0.02, 0.15);
    rug.renderOrder = 2; reg(rug, 0.86);

    const sW=1.8, sD=0.62, sH=0.44, bkH=0.50, bkD=0.13;
    const sZ=-1.22, bkZ=sZ-sD/2-bkD/2, sofaE=0x9A7858;
    ...
    box(lx,      fy+sH,  bkZ,               sW,    bkH, bkD,  0xCCB8A4, 0xCCAA88, 0xBBA880, sofaE, 1.0);
    const aW=0.13, aH=sH+bkH*0.65, aD=sD+bkD, aZc=sZ-bkD/2;
    for (const dx of [-(sW/2+aW/2), (sW/2+aW/2)])
        box(lx+dx, fy, aZc, aW, aH, aD, 0xE0CCBC, 0xBBA880, 0xCCAA88, sofaE, 1.0);

    const ctW=1.0, ctD=0.45, ctH=0.27, ctZ=0.68;
    const ctTop = new THREE.Mesh(new THREE.PlaneGeometry(ctW, ctD), face(0xC89858));
    ctTop.rotation.x = -Math.PI/2; ctTop.position.set(lx, ctH, ctZ);
    reg(ctTop, 1.0).renderOrder = 3;
    reg(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(ctW, 0.05, ctD)), ln(0x9A7858)), 0.95).position.set(lx, ctH, ctZ);
    for (const [dx,dz] of [[ctW/2-0.07,ctD/2-0.06],[ctW/2-0.07,-(ctD/2-0.06)],[-(ctW/2-0.07),ctD/2-0.06],[-(ctW/2-0.07),-(ctD/2-0.06)]]) {
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(lx+dx, fy, ctZ+dz), new THREE.Vector3(lx+dx, ctH-0.03, ctZ+dz),
        ]), ln(0x9A7858)), 0.92).renderOrder = 3;
    }
    reg(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(0.28, 0.04, 0.18)), ln(0x5C3C20)), 0.90).position.set(lx+0.22, ctH+0.025, ctZ-0.05);
});

// window-cracked + hallway-return crack lines
detail([[[-3.1,8.38,BW],[-2.65,8.05,BW],[-2.45,8.28,BW],[-2.1,7.80,BW]]], 820);
detail([
    [[-0.10,1.90,BW],[0.16,1.60,BW],[0.32,1.76,BW]],
    [[ 0.16,1.60,BW],[0.28,1.44,BW]],
], 820);

// mirror room: vanity + stool + mirror
roomDetail(540, reg => {
    const box = makeBox(reg);
    const rx = -3, fy = 3;

    const vW=1.2, vH=0.70, vD=0.42, vZ=-2+vD/2+0.40;
    box(rx, fy, vZ, vW, vH, vD, 0xF0DEC0, 0xCCAA88, 0xBBA880, 0x9A7858, 1.0);

    const dfz = vZ+vD/2+0.005;
    for (const dy of [fy+vH*0.33, fy+vH*0.66])
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(rx-vW/2+0.06, dy, dfz), new THREE.Vector3(rx+vW/2-0.06, dy, dfz),
    ...
    for (const [kx,ky] of [[rx-0.16,fy+vH*0.17],[rx+0.16,fy+vH*0.17],[rx-0.16,fy+vH*0.50],[rx+0.16,fy+vH*0.50]])
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(kx-0.025, ky, dfz), new THREE.Vector3(kx+0.025, ky, dfz),
        ]), ln(0x9A7858)), 0.92).renderOrder = 3;
    ...
    for (const [dx,dz2] of [[stW/2-0.06,stW/2-0.06],[stW/2-0.06,-(stW/2-0.06)],[-(stW/2-0.06),stW/2-0.06],[-(stW/2-0.06),-(stW/2-0.06)]])
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(rx+dx, fy, stZ+dz2), new THREE.Vector3(rx+dx, fy+stH-0.04, stZ+dz2),
        ]), ln(0x9A7858)), 0.90).renderOrder = 3;

    const vtY=fy+vH, vtZ=vZ+0.04;
    ...
    const mirW=0.80, mirH=0.90, mirCY=fy+vH+mirH/2+0.04, mirZ=-1.96;
    const mirBacking = new THREE.Mesh(new THREE.PlaneGeometry(mirW+0.14, mirH+0.14),
...
// hallway (kitchen): counter + stove + toaster + sink + upper cabinet
roomDetail(620, reg => {
    const box = makeBox(reg);
    const rx = 0, fy = 3;

    const ctrW=2.20, ctrH=0.88, ctrD=0.54, ctrZ=-2+ctrD/2+0.08;
    ...
    for (const [bx,bz] of [[stX-0.17,ctrZ-0.12],[stX+0.17,ctrZ-0.12],[stX-0.17,ctrZ+0.12],[stX+0.17,ctrZ+0.12]]) {
        const rpts=[], irpts=[];
        for (let i=0; i<=10; i++) { const a=i/10*Math.PI*2; rpts.push(new THREE.Vector3(bx+Math.cos(a)*0.09, fy+ctrH+0.009, bz+Math.sin(a)*0.09)); }
        for (let i=0; i<=8;  i++) { const a=i/8 *Math.PI*2; irpts.push(new THREE.Vector3(bx+Math.cos(a)*0.045, fy+ctrH+0.009, bz+Math.sin(a)*0.045)); }
    ...
    const tsX=rx-0.52, tsZ=ctrZ+0.03, tsW=0.22, tsH=0.15, tsD=0.17;
...
// bedroom: bed + headboard + pillows + blanket + nightstand
roomDetail(800, reg => {
    const box = makeBox(reg);
    const rx = -1.5, fy = 6;
    ...
    const hdH=0.62, hdD=0.12, hdZ=bZ-bL/2-hdD/2-0.01;
    box(rx, fy+bFH, hdZ, bW, hdH, hdD, 0x9A7040, 0xCCAA88, 0xBBA880, 0x9A7858, 1.0);

    const pY=fy+bFH+mTH, pZ=bZ-bL/2+0.46/2+0.12;
    for (const dx of [-bW/4, bW/4])
        box(rx+dx, pY, pZ, 0.88, 0.10, 0.46, 0xF8F4EE, 0xD8D2CA, 0xC8C2BA, 0xA8A09A, 1.0);

    const blL=bL*0.58, blZ=bZ+bL/2-blL/2;
    box(rx, fy+bFH+mTH, blZ, bW-0.14, 0.08, blL, 0x9AACC8, 0x506890, 0x405878, 0x304868, 1.0);

    const nsX=rx+bW/2+0.34, nsH=bFH+mTH+0.04, nsZ=bZ+0.10;
    box(nsX, fy, nsZ, 0.50, nsH, 0.44, 0xD4A870, 0xBBA880, 0xBBA880, 0x9A7858, 1.0);
    reg(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.CylinderGeometry(0.13, 0.09, 0.26, 6)), ln(0x9A7840)), 0.86).position.set(nsX, fy+nsH+0.13, nsZ-0.06);
});

// entrance: doormat + shoes + umbrella + key tray
roomDetail(320, reg => {
    const box = makeBox(reg);
    const rx = -3, fy = 0;

    const matMesh = new THREE.Mesh(new THREE.PlaneGeometry(0.60, 0.38), face(0x786054));
    matMesh.rotation.x = -Math.PI/2; matMesh.position.set(rx, fy+0.01, -1.62);
    matMesh.renderOrder = 2; reg(matMesh, 0.82);
...
// dining room: table + chairs + chandelier
roomDetail(480, reg => {
    const box = makeBox(reg);
    const rx = 0, fy = 0;
    ...
    for (const [dx,dz] of [[tW/2-0.09,tD/2-0.08],[tW/2-0.09,-(tD/2-0.08)],[-(tW/2-0.09),tD/2-0.08],[-(tW/2-0.09),-(tD/2-0.08)]])
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(rx+dx, fy, tZ+dz), new THREE.Vector3(rx+dx, fy+tH-0.03, tZ+dz),
        ]), ln(0x9A7858)), 0.90).renderOrder = 3;

    box(rx, fy+tH, tZ, 0.05, 0.14, 0.05, 0xF8F0DC, 0xEDE4C8, 0xE4DCB8, 0xB8B090, 1.0);
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(rx, fy+tH+0.14, tZ), new THREE.Vector3(rx+0.01, fy+tH+0.22, tZ),
    ...
    const addChair = (czC, backZ) => {
        box(rx, fy+legH, czC, cW, cSH, cD, cTop, cFront, cSide, cEdge, 1.0);
        box(rx, fy+legH+cSH, backZ, cW, cbH, cbD, cTop, cFront, cSide, cEdge, 1.0);
        for (const [dx,dz] of [[cW/2-0.05,cD/2-0.04],[cW/2-0.05,-(cD/2-0.04)],[-(cW/2-0.05),cD/2-0.04],[-(cW/2-0.05),-(cD/2-0.04)]])
            reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
                new THREE.Vector3(rx+dx, fy, czC+dz), new THREE.Vector3(rx+dx, fy+legH, czC+dz),
            ]), ln(cEdge)), 0.86).renderOrder = 3;
    };
    addChair(tZ+tD/2+0.38, tZ+tD/2+0.38+cD/2-cbD/2);
    addChair(tZ-tD/2-0.35, tZ-tD/2-0.35-cD/2+cbD/2);

    // Chandelier
    const cx=rx, cz=tZ, hy=2.85, hubY=1.78, armY=1.54, aR=0.44, dropY=1.10, centY=1.02;
    const gold=0xA8862A, crystal=0xC8E0F4;
    const mkRing = (r, y, segs) => {
        const pts = [];
        for (let i=0; i<=segs; i++) { const a=i/segs*Math.PI*2; pts.push(new THREE.Vector3(cx+Math.cos(a)*r, y, cz+Math.sin(a)*r)); }
        return pts;
    };
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(cx,hy,cz), new THREE.Vector3(cx,hubY,cz)]), ln(gold)), 0.90).renderOrder = 4;
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints(mkRing(0.17,   hubY,      16)), ln(gold)),    0.90).renderOrder = 4;
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints(mkRing(0.09,   hubY-0.07, 16)), ln(gold)),    0.88).renderOrder = 4;
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints(mkRing(aR*0.86,armY,      24)), ln(gold)),    0.90).renderOrder = 4;
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints(mkRing(aR*0.46,armY+0.04, 18)), ln(gold)),   0.86).renderOrder = 4;

    const armTips = Array.from({length:6}, (_,i) => { const a=i/6*Math.PI*2; return [Math.cos(a)*aR, Math.sin(a)*aR]; });
    for (const [dx,dz] of armTips) {
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(cx,hubY,cz), new THREE.Vector3(cx+dx,armY,cz+dz)]), ln(gold)), 0.90).renderOrder = 4;
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints(mkRing(0.06, armY, 10).map(v => new THREE.Vector3(v.x+dx, v.y, v.z+dz))), ln(gold)), 0.88).renderOrder = 4;
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(cx+dx,armY,cz+dz), new THREE.Vector3(cx+dx,dropY,cz+dz)]), ln(gold)), 0.90).renderOrder = 4;
    }
    for (let i=0; i<6; i++) {
        const a=(i+0.5)/6*Math.PI*2, rr=aR*0.86;
        const ox=cx+Math.cos(a)*rr, oz=cz+Math.sin(a)*rr, dBot=dropY+0.14;
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(ox,armY,oz), new THREE.Vector3(ox,dBot,oz)]), ln(gold)), 0.80).renderOrder = 4;
    ...
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(cx,hubY,cz), new THREE.Vector3(cx,centY+0.10,cz)]), ln(gold)), 0.90).renderOrder = 4;
    ...
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints(mkRing(crR, centY+0.03, 12)), ln(crystal)), 0.80).renderOrder = 4;
    const mkBulb = (bx,by,bz,r=0.09) => {
        const m = new THREE.Mesh(new THREE.SphereGeometry(r,8,6), new THREE.MeshBasicMaterial({ color: 0xffeaa0, transparent: true, opacity: 0 }));
        m.position.set(bx,by,bz); reg(m, 0.92).renderOrder = 4;
    };
    for (const [dx,dz] of armTips) mkBulb(cx+dx, dropY-0.04, cz+dz);
    mkBulb(cx, centY-0.04, cz, 0.07);
});

// study room: desk + books + lamp + chair
roomDetail(840, reg => {
    const box = makeBox(reg);
    const rx = 3, fy = 6;

    const dW=0.88, dH=0.74, dD=0.40, dZ=-2+dD/2+0.12;
    ...
    for (const [dx,dz2] of [[dW/2-0.06,dD/2-0.05],[dW/2-0.06,-(dD/2-0.05)],[-(dW/2-0.06),dD/2-0.05],[-(dW/2-0.06),-(dD/2-0.05)]])
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(rx+dx, fy, dZ+dz2), new THREE.Vector3(rx+dx, fy+dH-0.03, dZ+dz2),
    ...
    reg(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.CylinderGeometry(0.042, 0.036, 0.12, 7)), ln(0x9A8870)), 0.86).position.set(rx+0.30, fy+dH+0.06, dZ);
    reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
    ...
    reg(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.CylinderGeometry(0.06, 0.10, 0.08, 6)), ln(0xD4C890)), 0.88).position.set(rx+0.10, fy+dH+0.34, dZ+0.02);

    const legH=0.38, cW=0.42, cSH=0.08, cD=0.38, cbH=0.30, cbD=0.06, czC=dZ+dD/2+0.34;
    ...
    for (const [dx,dz2] of [[cW/2-0.05,cD/2-0.04],[cW/2-0.05,-(cD/2-0.04)],[-(cW/2-0.05),cD/2-0.04],[-(cW/2-0.05),-(cD/2-0.04)]])
        reg(new THREE.Line(new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(rx+dx, fy, czC+dz2), new THREE.Vector3(rx+dx, fy+legH, czC+dz2),
        ]), ln(0x9A7858)), 0.86).renderOrder = 3;

    reg(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(0.26, 0.20, 0.02)), ln(0x9A7858)), 0.78).position.set(rx, fy+dH+0.10, -1.97);
...
SEQUENCE.forEach(id => {
    ...
    if (pos.right) { div.style.right = pos.right; div.style.textAlign = 'right'; }
    div.style.top = pos.top;
    if (pos.noWrap) div.style.whiteSpace = 'nowrap';
    div.innerHTML  = `<span class="poem-block-text">${poemMap[id] || ''}</span>`;
...
const DAY_THEME = { color: '#1a110a', shadow: '0 0 14px rgba(190,218,248,0.65), 0 1px 3px rgba(255,250,245,0.55)' };
...
const STEP_THEMES = {
    ...
    'window': {
        color: '#FFF8F0',
        shadow: '-1px -1px 0 rgba(15,5,0,0.82), 1px -1px 0 rgba(15,5,0,0.82), -1px 1px 0 rgba(15,5,0,0.82), 1px 1px 0 rgba(15,5,0,0.82), 0 0 14px rgba(255,248,238,0.28)',
    },
    'bus-stop':         { color: '#FFFFFF', shadow: '0 1px 4px rgba(0,0,0,0.85), 0 0 18px rgba(255,255,255,0.15)' },
...
function stopStream(id) {
    if (activeStreams[id]) { clearInterval(activeStreams[id]); delete activeStreams[id]; }
    textBlocks[id].querySelector('.poem-block-text').innerHTML = (poemMap[id] || '').replace(/\n/g, '<br>');
}
...
function goTo(newStep) {
    ...
    camTo(ROOM_CAMERAS[id] || new THREE.Vector3(0, 6, 18), 0.035, ROOM_LOOK_AT[id] || TARGET);
    if (isLast) {
        setTimeout(() => camTo(OVERVIEW_CAM, 0.018, TARGET), 1400);
        setTimeout(() => runFinale(), 4200);
    }
...
// Finale
let _finaleActive = false;

function runFinale() {
    _finaleActive = true;
    poemLayer.style.transition = 'opacity 2s ease';
    poemLayer.style.opacity    = '0';

    const fin = document.getElementById('poem-finale');
    fin.innerHTML = '';

    const block = document.createElement('div');
    block.className = 'finale-block';
    fin.appendChild(block);

    setTimeout(() => { if (_finaleActive) fin.classList.add('visible'); }, 400);

    const fullText = SEQUENCE
        .map(sid => (poemMap[sid] || '').replace(/\n/g, ' ').trim())
        .join(' ');

    let wordDelay = 2200;
    fullText.split(/\s+/).filter(Boolean).forEach((word, wi, arr) => {
        const d = wordDelay;
        setTimeout(() => {
...
function stopFinale() {
    ...
    fin.innerHTML = '';
    poemLayer.style.transition = 'opacity 0.8s ease';
    poemLayer.style.opacity    = '';
}

function advance() { if (step < SEQUENCE.length - 1) goTo(step + 1); }
